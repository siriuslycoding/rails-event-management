class UserEventsController < ApplicationController
  ...
  def create
    ...
    @user_event.event.update(tickets_count: @user_event.event.tickets_count - 1)
    respond_to do |format|
      if @user_event.save
        format.html { redirect_to event_path(@user_event.event), notice: "User event was successfully created." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to user_events_path, status: :see_other, notice: "User event was successfully destroyed." }
  ...
  private
    ...
    def set_user_event
      @user_event = UserEvent.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def user_event_params
      params.expect(user_event: [ :user_id, :event_id ])
    end
end
